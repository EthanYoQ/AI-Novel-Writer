...
The V2 authoritative workbench uses the closed `/ai-novel` loopback channel with exactly five V2 endpoints: `state/read`, `proposal/list`, `command/preview`, `command/commit`, and `task/read`. Every request carries only the branded `WorkspaceId` plus a strictly typed payload: `command/preview` and `command/commit` accept a single-aggregate command whose `nextValue` is the complete next entity for `project`, `architecture`, `characters`, one `chapter`, or one `task`, while `task/read` adds only a task id. The Host resolves the canonical workspace directory through the Workspace registry and rejects unknown ids, any browser-supplied path, and any JSON patch. `command/preview` derives its entity-level diff from the current authoritative `NovelStore` value without writing; `command/commit` builds one validated single-aggregate `NovelChangeSet` and commits it through `applyChange`, which re-checks both revisions inside its exclusive transaction and records an idempotent audit. Failures return stable codes without local paths. Until the persistent proposal inbox lands, `proposal/list` returns a typed empty list.
