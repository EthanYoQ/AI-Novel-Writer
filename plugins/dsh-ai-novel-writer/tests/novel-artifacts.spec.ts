...
describe('NovelStore artifact version chain', () => {
  ...
  it('fails closed for a missing, wrong-kind, or cross-chapter artifact parent and never creates a final copy', async () => {
    const { store } = await initializedStore()
    ...
    const wrongParentRevision = await store.submitProposal(proposal({ changes: [{
      kind: 'artifact/revision', artifactId: 'lineage-revision-wrong', chapter: 1, parentArtifactId: 'lineage-draft-1',
      content: '错误父级修订稿。', summary: '尝试以 draft 作为 revision 父级。',
    }] }), signal)
    ...
    for (const submitted of [crossChapterReview, wrongParentRevision, missingFinal]) {
    ...
    const snapshot = await store.read(signal)
    expect(snapshot.artifacts.map(artifact => artifact.artifactId)).toEqual(['lineage-draft-1'])
