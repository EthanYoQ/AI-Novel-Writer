...
import { NovelContextBody } from '../src/client/context-view.tsx'
import type { NovelContextState } from '../src/client/context-store.ts'
...
const states: readonly [NovelContextState, string][] = [
  [{ status: 'loading', open: true }, '正在读取小说上下文'],
  [{ status: 'empty', open: true }, '当前会话不属于已注册工作区'],
  [{ status: 'not-initialized', open: true }, '尚未初始化 Harness 小说项目'],
  [{ status: 'error', open: true, message: 'invalid project' }, 'invalid project'],
  [{ status: 'disconnected', open: true }, 'Harness 连接已断开'],
]

describe('read-only novel context view', () => {
  it.each(states)('renders state %# with an explicit recovery path', (state, text) => {
    const html = renderToStaticMarkup(<NovelContextBody state={state} refresh={vi.fn()} selectChapter={vi.fn()} />)
    expect(html).toContain(text)
    expect(html).not.toContain('type="text"')
    expect(html).not.toContain('初始化项目')
    expect(html).not.toContain('批准修改')
  })

  it('renders identity, strategy, progress, plans, characters, and a bounded prose preview', () => {
    const state: NovelContextState = {
  ...
  it('defines a side drawer on wide screens and a full-width narrow layout', () => {
    ...
    try {
      ...
      expect(novelContextCss).toMatch(/@media\(max-width:760px\)/)
