...
> 执行：主线程用 superpowers:subagent-driven-development 分配独立行为切片；worker 不再派生。范围来自[现行规格](frontend-transition-specs.md)，不要从旧线程长对话重新规划。
...
规划基准：分支 codex/program-v3-refactor-thread-3，SHA `e9851798b3aeb8c8de5858d9236d3337074430fb`。未跟踪 U13 旅程、审计提案及截图必须保留，不 clean/reset/整体 stash；实际位置/新进度读唯一私有检查点，HEAD 后续变化不能回退。
...
薄切片成功不能掩盖核心阻断；S13 分段不是新增两片，S14C 准备不算最终通过。F05 的模型子门在 post-UI 后补齐；最终发布仍等全部必需门。
