...
`resolveGlobalDataRoots(userData, env?, home?)` resolves names without reading author files or creating directories. `AI_NOVEL_LEGACY_SOURCE_HOME` selects the legacy source; `AI_NOVEL_VELA_HOME` is its compatibility alias only. `AI_NOVEL_APP_DATA_HOME` selects the canonical target. Defaults are the legacy `.vela` and canonical `.ai-novel` directories under the home supplied to the locator. Existing Electron userData, partition, file origin and application single-instance identity stay unchanged.
