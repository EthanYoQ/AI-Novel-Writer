...
import type { LLMFinishReason } from './ipc-channels'
...
export function buildPlotTreeTask(sources: PlotTreeSourceBundle) {
  if (!hasUsablePlotTreeEventSource(sources)) throw new PlotTreeSourceError()
  if (sources.blueprints.length > PLOT_TREE_SOURCE_LIMITS.blueprints
    || sources.finalizedChapters.length > PLOT_TREE_SOURCE_LIMITS.finalizedChapters) {
    throw new PlotTreeSourceLimitError(PLOT_TREE_SOURCE_LIMITS.blueprints)
  }
  const facts = JSON.stringify(generationFacts(sources))
...
export interface PlotTreeDerivation { snapshot: PlotTreeSnapshot; kind: 'model' | 'normalized' | 'deterministic' }

export function derivePlotTreeSnapshot(content: string, sources: PlotTreeSourceBundle, generatedAt: string): PlotTreeDerivation {
  let parsed: Record<string, unknown>
  try { parsed = parsePlotTreeJSON(content) } catch { return { snapshot: deterministicPlotTreeSnapshot(sources, generatedAt), kind: 'deterministic' } }
  try { return { snapshot: strictSnapshot(parsed.tracks, sources, generatedAt), kind: 'model' } } catch {
    try { return { snapshot: normalizePlotTreeSnapshot(parsed, sources, generatedAt), kind: 'normalized' } } catch {
      return { snapshot: deterministicPlotTreeSnapshot(sources, generatedAt), kind: 'deterministic' }
    }
  }
}
