...
2. **把筛选权留给作者**：这次生成是作者主动点的，宁可多给几条、由他在预览里勾选或删掉，也不要因为「拿不准值不值得立条」就返回空数组；只要与这部作品相关就写出来。
...
- 拿不准某条值不值得立条时，照样输出它 —— 做减法是作者的活，不要替他筛。`,
  },
...
5. plotStructure 和 narrativePOV 请根据实际叙事特征判断，而非猜测`,
  ...
  // 便利贴 · AI 灵感抽卡
  //
  // 与上面所有模板的根本区别：**它的产出不进入任何创作事实**。
  // 抽出来的点子只会被追加进作者自己的便利贴，写稿、定稿、审稿
  // 一处都不会读到它 —— 所以这里的合同是「给作者一个人看」，
  // 不承担连贯性责任，也不必与正文口径一致。
  // ================================================================
  {
    key: 'inspiration_draw',
    name: '便利贴灵感抽卡',
    description: '按作者勾选的底稿与想法，一次给出数条彼此不同的构思方向（JSON）',
    systemRole: '你是长篇小说作者的灵感陪练。作者正在攒点子、找方向，你要给的是几条**彼此真正不同**的构思角度，帮他看见自己还没想到的那几条路。你不写正文，也不替他把故事定下来。',
    variables: {
      story_context: '作者勾选参与的故事底稿（可能为空）',
      references: '作者点名参考的角色 / 设定 / 蓝图条目（可能为空）',
      idea: '作者自己写下的构思想法（可能为空）',
      count: '这次要出几条点子',
    },
    content: `作者正在为下面这部作品攒灵感，请给出 {{count}} 条构思方向。

【故事底稿（作者勾选参与的部分，可能不完整）】
{{story_context}}
...
要求：
1. 每一条都要是**一件具体的事**：一个场景、一次转折、一处设定细节、一个会引出后果的动作或选择。不要写「可以增加冲突」这种空话。
2. 几条之间必须**方向不同** —— 不许把同一件事换几个说法凑数。宁可少给，也不要重复。
3. 每条都要接得上上面那些底稿，不要发明与它们冲突的设定或人物。
4. 这是给作者本人看的点子，不是小说正文：不铺陈描写，不写完整对白，不必连成段落。
5. 每条控制在 150–500 字之间；宁可短，也不要为凑字数注水。`,
    systemSuffix: `【输出合同】
- 只输出一个 JSON 对象，不要 Markdown 代码块、不要任何解释或客套话。
- 结构：{"cards":[{"idea":"这一条构思"}]}
- cards 的条数不超过作家要求的条数；每条 idea 不超过 500 字。
- 凑不满就少给几条，绝不为了凑数重复或灌水。`,
  },
]
