...
> ## v0.4.0 重大更新（发布前说明）
>
> 这个项目最初只是一次兴趣驱动的尝试，没想到获得了许多 Star 和真实使用反馈。为了回应这些用户，本轮对核心创作、数据、进程和安装更新链路进行了系统性 Code Review 与代码重构。
>
> v0.4.0 是一次重大更新：在正式 Release 发布后，建议安装版用户升级。它不是“零缺陷”承诺；正式安装包仍须完成云端 Windows 构建与资格验证后才会发布。NovelAI 的真实账户 Token 端到端调用也尚未在本仓库完成验证。
