...
export function releaseNotes(version) {
  return [
    `## 中文`,
    ``,
    `AI 小说作家 ${version} 是一次界面体验与模型配置入口的补丁更新。`,
    ``,
    `- 皮肤可见性：增强皮肤在各创作界面的可见性，并让经典主题保持主题一致性。`,
    `- 向量模型入口：顶部推荐硅基流动的 BAAI/bge-m3，并提供“免费模型注册链接”。`,
    ``,
    `本版本在同一个 Release 中提供五项资产：Windows x64 安装程序、其 blockmap、latest.yml、macOS Apple Silicon（ARM64）DMG 及其 SHA-256 校验文件。`,
    ``,
    ...
    `## English`,
    ``,
    `AI Novel Writer ${version} is a patch update for interface experience and the model-configuration entry.`,
    ``,
    `- Skin visibility: improves skin visibility across writing surfaces and preserves classic theme consistency.`,
    `- Embedding-model entry: recommends SiliconFlow's BAAI/bge-m3 at the top and provides a “Free model registration” link.`,
    ``,
    `This Release contains five assets: the Windows x64 installer, its blockmap, latest.yml, the macOS Apple Silicon (ARM64) DMG, and its SHA-256 checksum.`,
    ``,
