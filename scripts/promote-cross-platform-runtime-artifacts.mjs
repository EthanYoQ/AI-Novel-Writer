...
export function releaseNotes(version) {
  return [
    ...
    `- Windows 更新：点击“立即重启更新”后会静默完成安装并自动拉起新版本；同时新增针对正式 GitHub Release 的应用内更新端到端验收能力，用于校验从已安装正式版获得较新正式版的路径。`,
    ...
    ``,
    `- Windows x64：下载 \`ai-novel-writer-setup-${version}.exe\`；配套资产为 \`ai-novel-writer-setup-${version}.exe.blockmap\` 与 \`latest.yml\`，支持应用内更新。当前安装包未配置代码签名。`,
    ...
    `- Windows updates: “Restart and update now” now installs silently and relaunches the new version automatically. A formal GitHub Release in-app-update E2E path validates moving from an installed formal version to a newer formal version.`,
    ...
    ``,
    `- Windows x64: download \`ai-novel-writer-setup-${version}.exe\`; \`ai-novel-writer-setup-${version}.exe.blockmap\` and \`latest.yml\` support in-app updates. The installer is not code-signed.`,
