...
export function releaseNotes(version) {
  return [
    `## 中文`,
    ``,
    `AI 小说作家 ${version} 是一个补丁版本，修复以下问题：`,
    ``,
    `- #73：单章目标字数现在会真实生效；目标仍受所选模型的能力上限和自然边界约束，避免把 3000 字等目标误解为无止境续写、越写越多。`,
    `- #74：embedding 回填收到 HTML 或其他非 JSON 响应时，会提示检查配置、网关或鉴权，而不是抛出 \`Unexpected token\`；符合协议的合法 JSON 返回仍正常兼容。`,
    ``,
    `本版本在同一个 Release 资产列表中提供 Windows x64 与 macOS Apple Silicon（ARM64）安装包。`,
    ``,
    `- Windows x64：下载 \`ai-novel-writer-setup-${version}.exe\`，支持应用内更新。`,
    ...
    `## English`,
    ``,
    `AI Novel Writer ${version} is a patch release for the following fixes:`,
    ``,
    `- #73: makes a single chapter's target word count actually take effect. Targets remain bounded by the selected model's output capacity and natural completion boundaries, so a 3,000-word target does not turn into ever-longer continuation.`,
    `- #74: when embedding backfill receives HTML or another non-JSON response, reports configuration, gateway, or authentication troubleshooting instead of \`Unexpected token\`; valid JSON responses remain compatible.`,
    ``,
    `This release includes Windows x64 and macOS Apple Silicon (ARM64) installers in one Release asset list.`,
    ``,
    `- Windows x64: download \`ai-novel-writer-setup-${version}.exe\`; it supports in-app updates.`,
