...
describe('cross-platform runtime artifact promotion workflow contract', () => {
  ...
  it('is manual-only and publishes exactly one verified formal Release from both cloud artifacts', () => {
    ...
    expect(promotion).toContain('verifyQualificationBundle')
    expect(promotion).toContain('contractSha256')
    expect(promotion).toContain('ledgerSha256')
    expect(promotion).toContain("assertCreatedTagCommit(fetcher, api, headers, ready, 'before draft creation')")
    expect(promotion).toContain("assertTagCommit(fetcher, api, headers, ready, 'before draft creation')")
    expect(promotion).toContain('A Git tag exists without its matching draft Release; stop for manual recovery')
    ...
    expect(promotion).toContain('Read back published release')
    expect(promotion).toContain('validatePublishedRelease')
    expect(promotion).toContain("remote?.state === 'uploaded'")
