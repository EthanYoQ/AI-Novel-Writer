...
describe('cross-platform artifact promotion planner', () => {
  ...
  it('generates bilingual v0.7.1 patch-release notes for skin visibility, classic-theme consistency, and the vector entry', () => {
    const body = releaseNotes('0.7.1')

    expect(body).toContain('## 中文')
    expect(body).toContain('## English')
    expect(body).toContain('AI 小说作家 0.7.1')
    expect(body).toContain('AI Novel Writer 0.7.1')
    expect(body).toContain('补丁更新')
    expect(body).toContain('patch update')
    expect(body).toContain('皮肤可见性')
    expect(body).toContain('skin visibility')
    expect(body).toContain('经典主题')
    expect(body).toContain('classic theme')
    expect(body).toContain('主题一致性')
    expect(body).toContain('theme consistency')
    expect(body).toContain('向量模型入口')
    expect(body).toContain('Embedding-model entry')
    expect(body).toContain('硅基流动')
    expect(body).toContain('SiliconFlow')
    expect(body).toContain('BAAI/bge-m3')
    expect(body).toContain('免费模型注册链接')
    expect(body).toContain('Free model registration')
    expect(body).toContain('五项资产')
    expect(body).toContain('five assets')
    expect(body).toContain('ai-novel-writer-setup-0.7.1.exe')
    expect(body).toContain('ai-novel-writer-setup-0.7.1.exe.blockmap')
    expect(body).toContain('latest.yml')
    expect(body).toContain('ai-novel-writer-mac-arm64-0.7.1-installer.dmg')
    expect(body).toContain('ai-novel-writer-mac-arm64-0.7.1-installer.dmg.sha256')
