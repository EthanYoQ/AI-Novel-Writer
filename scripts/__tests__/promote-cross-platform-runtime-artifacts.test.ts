...
describe('cross-platform artifact promotion planner', () => {
  ...
  it('generates bilingual v0.7.0 major-release notes for skins and release-update qualification', () => {
    ...
    expect(body).toContain('in-app-update E2E path')
    expect(body).toContain('静默完成安装并自动拉起新版本')
    expect(body).toContain('installs silently and relaunches the new version automatically')
