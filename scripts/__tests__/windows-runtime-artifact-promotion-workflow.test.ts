...
describe('Windows runtime artifact promotion workflow contract', () => {
  ...
  it('verifies remote assets before atomically claiming the tag and formally publishing', () => {
    ...
    expect(publish.indexOf('verifyRemoteReleaseAssets')).toBeGreaterThan(-1)
    expect(publish.indexOf('claimLightweightTag')).toBeGreaterThan(publish.indexOf('verifyRemoteReleaseAssets'))
    expect(publish.indexOf('finalizeVerifiedDraft')).toBeGreaterThan(publish.indexOf('claimLightweightTag'))
  })
})
