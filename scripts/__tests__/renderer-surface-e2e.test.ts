...
describe('renderer surface E2E runner contract', () => {
  ...
  it('documents reproducible same-state evidence instead of the obsolete walnut mock comparison', () => {
    const qa = readFileSync(resolve(process.cwd(), 'design-qa.md'), 'utf8')

    expect(qa).toContain('AI_NOVEL_RENDERER_VISUAL_EVIDENCE_DIR')
    expect(qa).toContain('1440x900')
    expect(qa).toContain('open-project')
    expect(qa).toContain('light')
    expect(qa).toContain('galaxy')
    expect(qa).toContain('paper')
    expect(qa).toContain('dark')
    expect(qa).toContain('计算样式合同')
    expect(qa).toContain('人工视觉检查')
    expect(qa).toContain('classic')
    expect(qa).toContain('anime')
    expect(qa).toContain('custom')
    expect(qa).toContain('关闭并重新启动')
    expect(qa).toContain('run-state.json')
    expect(qa).toContain('in-progress')
    expect(qa).toContain('completed')
    expect(qa).toContain('原子')
    expect(qa).toContain('Electron 正常退出')
    expect(qa).toContain('Node ABI')
    expect(qa).toContain('failed')
    expect(qa).toContain('每张截图')
    expect(qa).not.toContain('walnut')
    expect(qa).not.toContain('brass')
    expect(qa).not.toMatch(/[A-Z]:\\/)
  })
})
